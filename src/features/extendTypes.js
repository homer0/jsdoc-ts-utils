 ...
 * @property {string[]} lines      The list of lines from the comment that can be used on the type
 *                                 that it's being extended.
 * @property {number}   usedLines  This counter gets modified during parsing and it represents how
 *                                 many of the `lines` were used: if the type with the intersection
...
class ExtendTypes {
  ...
  constructor(events, EVENT_NAMES) {
    ...
    /**
     * A list with the information of comments that extend/agument types with intersections.
     *
  ...
  /**
   * Given a list of types from an instersection definition, this methods tries to find a comment
   * that extends one of those types.
   * If the method finds only one comment for a type, it returns it, otherwise, it returns `null`:
  ...
  /**
   * Parses a JSDock comment block for a type definition that extends a type with intersection.
   *
  ...
  /**
   * This is called every time a new JSDoc comment block is found on a file. It validates if
   * the block uses intersection or if it's a `typedef` that extends another type in order to save
   * it to be parsed later.
   *
  ...
  /**
   * Replaces the JSDoc comment blocks for type definitions that extend types with intersections
   * with empty lines.
   *
  ...
  /**
   * This is called after all the JSDoc comments block for a file were found and the plugin is
   * ready to make changes.
   * The method first _"fixes"_ the `typedef` statements with intersections and then removes
   * the comments for types that extend another types.
   *
  ...
  /**
   * Parses the comments with intersections, validate if there's a type extending them from where
   * they can take "lines", or if they should be transformed into unions.
   *
    ...
    return this._commentsWithIntersections.reduce(
      (acc, comment) => {
        ...
        if (commentWithProps) {
          ...
          /**
           * For each line of the type that extends the definition, check if it doesn't already
           * exists on the comment (this can happen with `access` or `memberof` statements),
           * and if it doesn't, it not only adds it but it also increments the counter that tracks
           * how many lines of the comment are being used.
           *
           * The lines that are moved are counted so the class can later replace the comment
           * with enough empty lines so it won't mess up the line number of the rest of the types.
           * For example: If we had a type with intersection with a single line, a type that
           * extends it with 3 lines lines with `property` and one with `access` (plus the
           * `typedef` and the `extends`/`augments`) and the intersection type already has
           * `access`, the class would replace the comment with `5` empty lines. The comment had
           * a total of 8 lines, 3 `property`, `typedef`, `extends`/`augments`, `access` and the
           * opening and closing lines; but the properties were moved to another type.
           *
           * @ignore
           */
